...
def encounter(player: Player, location: Dict[str, Area]) -> None:
    ...
    if location == AREA['Haunted House'] or location == AREA[
        ...
        if start == 1:
            ...
            print(f'The wild {wild_poke.name} appears.')
            encounter_options(player, wild_poke)
    if location == AREA['Box'] or location == AREA['Open']:
        if 'Gold Medal' in player.items:
            print('Huh? your Golden Medal is vigoriously shacking the closer you get to the box, when you get a closer look, there is a spot for the Medal. Will you insert the Medal into the slot of the box?')
            insert_option = input('> ')
...
def Giratina(player: Player, location: Dict[str, Area]) -> None:
    print('The box opens up and sucked you inside. All you could see was just blue and purple everywhere. You realize that the fairy tail of the Distortion World was true. Then, an humongus looking Pokemon appears out of thin air. It is the legendary Pokemon named Giratina, and it does NOT look happy!')
    ...
    if Distortion.fainted == True:
        print('Congratulations! You have a chance to Catch this Pokemon!')
        player.items.remove('Gold Medal')
...
def catch_giratina(player: Player, Distortion: Pokemon) -> None:
    ...
    while True:
        ...
        if attempts < attempt:
            ...
            action = input('> ')
            if action == 'Run':
                if 'Pokeball' not in player.items and 'Greatball' not in player.items and 'Ultraball' not in player.items and 'Masterball' not in player.items:
                    print('Too bad that you was not prepared for this...')
                    break
                print(
                    'You cannot run away from a Legendary unless you run out of Pokeball`s!'
                )
            elif action == 'Catch':
                print(player.items)
...
def encounter_options(player: Player, wild_poke: Pokemon) -> None:
    if len(player.party) < 5:
        ...
        print(
            f'A wild {wild_poke.name} appeared but you had no space for it and ran!'
        )
...
def battlecounter_options(player: Player, location: Dict[str, Area]):
    while True:
        print('Welcome to the Battle Counter! What Tower would you like to battle? or press q to quit.')
        print('Bronze')
        print('Silver')
        print('Gold')
        action = input('> ')
...
def white_out(location: Dict[str, Area], player: Player) -> None:
    ...
    print('You lost....You were wiped out!')
...
def main() -> None:
    location = AREA['Safari Counter']
    print('Welcome to the world of pokemon, you start out with $1000 and 5 Pokeballs to catch some interesting pokemons, and of course you will start with a starter pokemon, but first!')
